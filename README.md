...
- `/feature-plan <feature>` — plan a feature from an existing spec document (creates execution plan + tickets)
- `/feature-init <feature>` — scaffold a feature folder with a stub spec file
...
- `/feature-status <feature>` — show current feature ticket progress
...
./docs/technical-specs/
  <feature>/
    01-master-spec.md
    02-execution-plan.md      (generated by /feature-plan)
...
The planner classifies the spec scope and may emit `BLOCKED` if critical technical detail is genuinely missing. When that happens, add the required context and run `/feature-plan <feature>` again to resume.
...
1. Run `/feature-init <feature>` to scaffold a stub `01-master-spec.md`.
2. Fill in the spec document.
3. Run `/feature-plan <feature>` to generate the execution plan and tickets.
4. Run `/feature-validate <feature>` to check for structural issues.
5. Run `/feature-start <feature>` or `/feature-next <feature>` to begin implementation.
6. If execution stops because a ticket is blocked or needs fix, use `/feature-blocked`, `/feature-needs-fix`, `/feature-done`, `/feature-start`, or `/feature-next` to steer it.
