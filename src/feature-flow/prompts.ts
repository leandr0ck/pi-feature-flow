...
export function buildTesterPrompt(
  ...
  return [
    ...
    "3. Fill in the 'Hidden test dependencies' section in the tester notes template.",
    "   Check every box that applies — this tells the worker what assumptions the tests make",
    "   about response shape, auth semantics, database state, and derived data.",
    "   Be honest: if the test uses raw DB inserts + an HTTP call, check both boxes.",
    "4. Do NOT run the test suite. Leave execution to the Worker.",
    `5. Write the tester notes file to: ${testerNotesPath}`,
    `6. Create or update the handoff log at: ${handoffLogPath}`,
    `7. Write the structured tester handoff JSON to: ${testerHandoffPath}`,
    "",
  ...
  lines.push(
    ...
    ...(testerNotesPath
      ? [
          ...
          "   Check the 'Hidden test dependencies' section in the tester notes — it tells you which",
          "   assumptions the tests make about response shape, auth semantics, and database state.",
          "2. Run the tests first and confirm the current failing state before implementing.",
          "   Show the failing output verbatim. Do not skip this step.",
          "3. ⚠️ FAILURE TRIAGE: Classify each failing assertion into exactly one bucket:",
          "   - implementation failure — the code path exists but logic is wrong",
          "   - fixture/setup failure — test data, DB state, or setup is incomplete",
          "   - contract/response-shape mismatch — test and API disagree on response format",
          "   - authorization/role mismatch — wrong permissions or missing auth setup",
          "   - duplicate data / idempotency issue — repeated inserts hitting unique constraints",
          "   - precondition missing (feature code never reached) — endpoint returned ok but entity is absent",
          "   - unknown / convention-dependent — behavior depends on undocumented conventions",
          "   ",
          "   ⚠️ FIXTURE-DEPTH CHECK: If the test does BOTH of:",
          "     - calls an HTTP endpoint or full service boundary",
          "     - uses raw DB inserts, low-level factories, or partial fixtures",
          "   then warn: 'This test may rely on fixtures that bypass side effects required by",
          "   the endpoint under test.' Non-idempotent setup blocks on unique constraints are",
          "   the most common symptom — check for duplicate inserts or shared beforeAll state.",
          "   ",
          "   ⚠️ RESPONSE-SHAPE CHECK: Look for patterns where tests probe multiple alternative",
          "   shapes for the same entity (e.g. a?.b || a?.c || a?.d, mixed wrapped/unwrapped",
          "   payloads). If present, warn: 'Response contract appears ambiguous. Verify actual",
          "   endpoint contract before implementing.' Do NOT overfit to one guessed shape.",
          "   ",
          "   ⚠️ PRECONDITION CHECK: Before concluding a failure means feature code is wrong:",
          "     - Did the endpoint return a successful status (2xx)?",
          "     - Does the entity under test actually exist in the response?",
          "     - Was the relevant code branch reached?",
          "     - Does required related data exist?",
          "   If the tested entity is entirely absent, classify as 'precondition missing' (setup",
          "   problem), NOT 'business logic failure'.",
          "   ",
          "   ⚠️ UNKNOWN CONVENTION CHECK: If you need to assume behavior not inferable from",
          "   the ticket, nearby code, docs, or existing tests, mark the failure as",
          "   'convention-dependent' and search for nearby examples or project guidance files",
          "   before making edits.",
          "4. Only after failure triage: if all failures are from one bucket, proceed.",
          "   If failures span 2+ buckets, pause and write a short root-cause summary before",
          "   editing any files. Example: 'Group A: implementation gaps — Group B: fixture",
          "   issues — Group C: contract mismatch'.",
          "5. Follow the Implementation Notes in the ticket, in order.",
          "   Each note is a required step, not a suggestion.",
          "6. Implement the minimum code that makes the existing tests pass.",
          "   Do not add code that is not required by the tests.",
          "   If the failure was classified as fixture/setup, fix the test setup, not the implementation.",
          "   If the failure was classified as contract/response-shape, verify the actual endpoint",
          "   contract via runtime inspection (log the response body) before editing either layer.",
          "7. Run the tests. Show GREEN output verbatim.",
          "8. Run typecheck. Show clean output or fix every error.",
          "9. ⚠️ If the second run still has multiple failures, do NOT keep editing linearly.",
          "   Stop and perform ROOT CAUSE GROUPING before more code changes:",
          ...
          `10. Update the Worker section in ${handoffLogPath} with files changed, technical decisions, and risks.`,
          `11. Write the structured worker handoff JSON to ${workerHandoffPath}.`,
          "12. Verify every Acceptance Criterion is met.",
          "13. CONFIDENCE GATE: Before saying APPROVED, state confidence separately for:",
          ...
          "    If implementation confidence is high but test confidence is low due to setup ambiguity,",
          "    return NEEDS-FIX with explicit cause categories, not a vague summary.",
          "14. Say APPROVED only if: tests pass + typecheck clean + all ACs satisfied + handoff log updated + worker handoff JSON written.",
        ]
      : [
          ...
          "4. ⚠️ FAILURE TRIAGE: Classify each failing assertion into exactly one bucket:",
          "   - implementation failure — the code path exists but logic is wrong",
          "   - fixture/setup failure — test data, DB state, or setup is incomplete",
          "   - contract/response-shape mismatch — test and API disagree on response format",
          "   - authorization/role mismatch — wrong permissions or missing auth setup",
          ...
          "   'convention-dependent' and search for nearby examples or project guidance files",
          "   before making edits.",
          "5. Only after failure triage: if all failures are from one bucket, proceed.",
          "   If failures span 2+ buckets, pause and write a short root-cause summary before",
          "   editing any files. Example: 'Group A: implementation gaps — Group B: fixture",
          "   issues — Group C: contract mismatch'.",
          "6. Implement the minimum code that makes those tests pass.",
          "   If the failure was classified as fixture/setup, fix the test setup, not the implementation.",
          "   If the failure was classified as contract/response-shape, verify the actual endpoint",
          "   contract via runtime inspection (log the response body) before editing either layer.",
          "7. Run the tests again. Show GREEN output verbatim.",
          "8. Run typecheck. Fix every error.",
          "9. ⚠️ If the second run still has multiple failures, do NOT keep editing linearly.",
          "   Stop and perform ROOT CAUSE GROUPING before more code changes:",
          "   - Group A: implementation gaps",
          "   - Group B: fixture/setup issues",
          ...
          "   - Group D: unresolved / convention-dependent",
          "   This breaks the 'patch one symptom at a time' loop.",
          `10. Create or update ${handoffLogPath} with a Worker section describing the implementation and risks.`,
          `11. Write the structured worker handoff JSON to ${workerHandoffPath}.`,
          "12. CONFIDENCE GATE: Before saying APPROVED, state confidence separately for:",
          "    - Implementation correctness: <high | medium | low>",
          "    - Fixture correctness:       <high | medium | low>",
          "    - Contract alignment:        <high | medium | low>",
          "    - Test pass state:           <passing | failing>",
          "    If implementation confidence is high but test confidence is low due to setup ambiguity,",
          "    return NEEDS-FIX with explicit cause categories, not a vague summary.",
          "13. Say APPROVED only if: tests pass + typecheck clean + all ACs satisfied + handoff log updated + worker handoff JSON written.",
        ]),
    "",
    "### What is NOT allowed:",
