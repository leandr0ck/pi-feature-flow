...
export function renderTicketTemplate(input: TicketTemplateInput): string {
  ...
  return [
    ...
    "- Files: <repo-relative-implementation-path>, <repo-relative-test-path>",
    "",
...
export function buildTicketTemplateInstructions(_availableProfiles?: string[]): string {
  return [
    ...
    "Every ticket must include at least one writable test file path in `- Files:` so Tester/Worker can complete the red phase legally.",
    "",
    "## Implementation Notes",
