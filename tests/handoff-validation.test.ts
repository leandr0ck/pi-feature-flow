...
describe("handoff validation", () => {
  ...
  it("accepts valid worker and chief artifacts", async () => {
    const dir = await makeTempDir();
    dirs.push(dir);
    ...
    const chiefLog = path.join(dir, "chief-handoff-log.md");
    const chiefJson = path.join(dir, "chief-handoff.json");
    ...
    await writeFile(chiefLog, "# Handoff Log — STK-001\n\n## Chief\n- promoted learnings\n", "utf8");
    await writeFile(chiefJson, JSON.stringify({ ticketId: "STK-001", phase: "chief", status: "APPROVED", promotedToFeatureMemory: ["reuse helper"], reusablePatterns: ["thin slices"], continuationAdvice: ["read feature memory"] }, null, 2), "utf8");
    const chiefResult = await validateChiefArtifacts(context, memory, chiefLog, chiefJson);
    expect(chiefResult.ok).toBe(true);
  });
});
